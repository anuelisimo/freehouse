...
const NAV = [
  ...
  { href: '/graficos',    label: 'CHARTS',   icon: IconCharts },
...
function IconOverview({ active }: { active: boolean }) {
  ...
  return (
    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke={c} strokeWidth="1.5">
...
function IconCharts({ active }: { active: boolean }) {
  const c = active ? 'var(--accent)' : 'var(--text3)'
  return (
    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke={c} strokeWidth="1.5">
      <path d="M3 12l4-4 4 4 4-6 4 3"/><path d="M3 20h18"/>
    </svg>
  )
}
